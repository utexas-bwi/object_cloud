...
public:
  ...
  typedef typename pcl::SampleConsensusModelLine<PointT>::PointCloudPtr
      PointCloudPtr;
  typedef typename pcl::SampleConsensusModelLine<PointT>::PointCloudConstPtr
      PointCloudConstPtr;
  ...
  SACModelEdge(const PointCloudConstPtr &cloud,
               std::vector<Eigen::Vector3f> &invalid_lines, float min_length,
               float min_density, bool random = false)
      : invalid_lines_(invalid_lines),
        pcl::SampleConsensusModelLine<PointT>(cloud, random),
        axis_(Eigen::Vector3f::Zero()), eps_cosine_(0.0),
        min_length_(min_length), min_density_(min_density) {
   ...
   * and the given axis (in radians).
    */
  inline void setEpsCosine(const double ea) { eps_cosine_ = ea; }
  ...
  void selectWithinDistance(const Eigen::VectorXf &model_coefficients,
    ...
    Eigen::Vector4f line_pt(model_coefficients[0], model_coefficients[1],
    ...
    for (size_t i = 0; i < indices_->size(); ++i) {
      ...
      // norm(p2-p1)
      double sqr_distance =
          (line_pt - input_->points[(*indices_)[i]].getVector4fMap())
              .cross3(line_dir)
              .squaredNorm();

      const Eigen::Vector3f &pt =
          input_->points[(*indices_)[i]].getVector3fMap();
      bool oncliff =
          pt(min) > (line_pt(min) +
                     (pt(max) - line_pt(max)) / line_dir(max) * line_dir(min) -
                     threshold);
...
protected:
  ...
  bool isModelValid(const Eigen::VectorXf &model_coefficients) override {
    if (!pcl::SampleConsensusModelLine<PointT>::isModelValid(
            model_coefficients))
      return (false);

    // Obtain the line direction
    Eigen::Vector3f line_dir(model_coefficients[3], model_coefficients[4],
    ...
    // Obtain the line point and direction
    Eigen::Vector3f line_pt(model_coefficients[0], model_coefficients[1],
    ...
    for (size_t i = 0; i < indices_->size(); ++i) {
      ...
      bool oncliff =
          pt(min) >
          (line_pt(min) +
           (pt(max) - line_pt(max)) / line_dir(max) * line_dir(min) - 0.01);

      // Calculate the distance from the point to the line
      // D = ||(P2-P1) x (P1-P0)|| / ||P2-P1|| = norm (cross (p2-p1, p2-p0)) /
      // norm(p2-p1)
      double sqr_distance =
          (line_pt - input_->points[(*indices_)[i]].getVector3fMap())
              .cross(line_dir)
              .squaredNorm();
