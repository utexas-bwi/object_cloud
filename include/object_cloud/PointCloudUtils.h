...
public:
  ...
  extractSurface(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
                 ...
                 const Eigen::Affine3f &camToStraightCam, float xScale,
                 float yScale, float zScale, ros::Publisher &publisher);

  static std::vector<Eigen::Vector2f>
  surfaceOccupancy(std::vector<visualization_msgs::Marker> all_objects,
