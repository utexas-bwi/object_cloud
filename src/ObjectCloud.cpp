...
pair<bool, Object> ObjectCloud::addObject(const ImageBoundingBox &bbox,
  ...
  float depth = depth_image.at<uint16_t>(bbox.y + bbox.height / 2.,
                                         bbox.x + bbox.width / 2.) /
                1000.;
  ...
  Eigen::Vector3f world =
      cam_to_map * Eigen::Vector3f(camera_x, camera_y, camera_z);
  ...
  if (!cloud.pts.empty()) {
    float sqRadius = NEIGHBOR_THRESH_X * NEIGHBOR_THRESH_X +
                     NEIGHBOR_THRESH_Y * NEIGHBOR_THRESH_Y +
                     NEIGHBOR_THRESH_Z * NEIGHBOR_THRESH_Z;
    std::vector<std::pair<size_t, float>> indices_dists;
    RadiusResultSet<float, size_t> resultSet(sqRadius, indices_dists);
    cloud_index.findNeighbors(resultSet, candidatePoint,
...
vector<Object> ObjectCloud::searchBox(const Eigen::Vector3f &origin,
  ...
  Eigen::Vector3f min(origin(0) - scale(0), origin(1) - scale(1),
                      origin(2) - scale(2));
  Eigen::Vector3f max(origin(0) + scale(0), origin(1) + scale(1),
                      origin(2) + scale(2));
