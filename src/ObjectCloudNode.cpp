...
ObjectCloudNode::ObjectCloudNode(ros::NodeHandle node)
  ...
  viz_pub = node.advertise<visualization_msgs::MarkerArray>("/object_cloud/markers", 1, true);
  cloud_pub = node.advertise<octomap_msgs::Octomap>("/object_cloud/cloud", 1);
  surfacecloud_pub = node.advertise<PointCloudT>("/object_cloud/surfacecloud", 1);
  surface_marker_pub = node.advertise<visualization_msgs::MarkerArray>("/object_cloud/surfacemarker", 1);
  bbox_pub = node.advertise<visualization_msgs::MarkerArray>("/object_cloud/box", 1);
#endif
...
void ObjectCloudNode::advertise_services() {
  //TODO: Switch to standard service naming convention: http://wiki.ros.org/ROS/Patterns/Conventions#Topics_.2BAC8_services
  clear_octree_server = node.advertiseService("/object_cloud/clear_octree", &ObjectCloudNode::clear_octree, this);
  get_entities_server = node.advertiseService("/object_cloud/get_entities", &ObjectCloudNode::get_entities, this);
  get_bounding_boxes_server = node.advertiseService("/object_cloud/get_bounding_boxes", &ObjectCloudNode::get_bounding_boxes,
                                             this);
  get_objects_server = node.advertiseService("/object_cloud/get_objects", &ObjectCloudNode::get_objects, this);
  get_surface_server = node.advertiseService("/object_cloud/get_surfaces", &ObjectCloudNode::get_surfaces, this);
  surface_occupancy_server = node.advertiseService("/object_cloud/get_surface_occupancy",
                                            &ObjectCloudNode::get_surface_occupancy,
                                            this);
}
...
bool ObjectCloudNode::clear_octree(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res) {
...
ObjectCloudNode::get_objects(villa_object_cloud::GetObjects::Request &req, villa_object_cloud::GetObjects::Response &res) {
  ...
  try {
    frameToMapTransform = tf_buffer.lookupTransform("map", req.search_box.header.frame_id,
                                                    req.search_box.header.stamp, ros::Duration(0.1));
...
bool ObjectCloudNode::get_bounding_boxes(villa_object_cloud::GetBoundingBoxes::Request &req,
  ...
  try {
    frameToMapTransform = tf_buffer.lookupTransform("map", req.search_box.header.frame_id,
                                                    req.search_box.header.stamp, ros::Duration(0.2));
  } catch (tf2::TransformException &ex) {
    ROS_ERROR("%s", ex.what());
    return false;
  }
...
bool ObjectCloudNode::get_surfaces(villa_object_cloud::GetSurfaces::Request &req,
  ...
  try {
    camToMapTransform = tf_buffer.lookupTransform("map", "head_rgbd_sensor_rgb_frame", ros::Time(0),
                                                  ros::Duration(0.1));
    camToBaseTransform = tf_buffer.lookupTransform("base_link", "head_rgbd_sensor_rgb_frame", ros::Time(0),
                                                   ros::Duration(0.1));
    baseToHeadPanTransform = tf_buffer.lookupTransform("head_pan_link", "base_link", ros::Time(0),
                                                       ros::Duration(0.1));
  } catch (tf2::TransformException &ex) {
    ROS_ERROR("%s", ex.what());
    return false;
  ...
  for (int surfaceId = 0; surfaceId < req.z_values.size(); surfaceId++) {
    ...
    for (int i = 0; i < cloudReq->result->points.size(); ++i) {
      // If z_values[surfaceId] is negative, we can assume user doesn't want z restrictions
      if (req.z_values[surfaceId] >= 0 &&
          std::abs(cloudReq->result->points[i].z - req.z_values[surfaceId]) > req.z_eps) {
        continue;
      }
    ...
    //boost::optional<visualization_msgs::Marker> surface = PointCloudUtils::extractSurface(cloudReq->result, camToMap, camToStraightCam, req.x_scale, req.y_scale);
    boost::optional<visualization_msgs::Marker> surface = PointCloudUtils::extractSurface(transformed,
                                                                                          camToMap,
                                                                                          transform *
                                                                                          camToStraightCam,
                                                                                          req.x_scales[surfaceId],
                                                                                          req.y_scales[surfaceId],
                                                                                          req.z_scales[surfaceId],
                                                                                          surfacecloud_pub);
    ...
    float zval = ((camToMap * (transform * camToStraightCam).inverse()).inverse() *
                  Eigen::Vector3f(marker.pose.position.x, marker.pose.position.y, marker.pose.position.z))(2) +
...
bool ObjectCloudNode::get_surface_occupancy(villa_object_cloud::GetSurfaceOccupancy::Request &req,
  ...
  std::vector<Eigen::Vector2f> occupancy = PointCloudUtils::surfaceOccupancy(objects, req.surface,
                                                                             Eigen::Vector2f(req.x_bounds[0],
                                                                                             req.x_bounds[1]),
                                                                             Eigen::Vector2f(req.y_bounds[0],
                                                                                             req.y_bounds[1]));
